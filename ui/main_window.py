...
class MainWindow:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self._MAX_UNDO = 20  # Undo geçmişi maksimum 20 adım
        ...
        self._preview_debounce_id = None   # önizleme güncellemesi için debounce
        ...
        # ── Widget'ları settings'den başlat ──
    ...
    def _init_widgets_from_settings(self) -> None:
        """AppState değerlerini tüm tab widget'larına uygula."""
        try:
    ...
    def on_file_selected(self, idx: int) -> None:
        """Dosya seçildiğinde önizleme güncelle ve karşılaştırma modunu sıfırla."""
    ...
    def start_convert(self) -> None:
        """Mevcut görseli dönüştür, buffer'a yaz, önce/sonra karşılaştırmasını göster."""
        ...
        def _worker():
            try:
                # Undo için mevcut görseli kaydet
                if path in self._buffer:
    ...
    def save_buffer(self) -> None:
        ...
        def _worker():
            ...
            for idx, (path, img) in enumerate(list(self._buffer.items()), start=1):
                try:
                    # rembg görselleri şeffaflık korunması için PNG olarak kaydedilir
    ...
    def upscale_selected(self) -> None:
        """Mevcut görseli upscale et ve önce/sonra karşılaştırmasını göster."""
        result = self._get_current_image()
        if result is None:
            self.log("⚠️ Önce bir görsel açın")
            return
    ...
    def remove_background_selected(self) -> None:
        """Mevcut görselin arka planını kaldır ve önce/sonra karşılaştırmasını göster."""
        if not RemBGService.is_available():
            self.log("❌ rembg kütüphanesi bulunamadı. 'pip install rembg' komutunu çalıştırın.")
            return
        ...
        def _worker():
            try:
                ...
                # Arka plan kaldırılan görsel __rembg__ anahtarıyla saklanır
                # PNG formatında kaydedilerek şeffaflık korunur
                rembg_key = path + "__rembg__"
                self._buffer[rembg_key] = removed
    ...
    def set_ai_mode_from_split(self, mode: str) -> None:
        """AI modunu değiştir ve tüm tab'lara bildir."""
    ...
    def _sync_state(self) -> None:
        """ConverterTab ve EditorTab'dan AppState'i güncelle."""
        try:
            s = self._get_converter_state()
    ...
    def _push_undo(self, path: str, img: Image.Image) -> None:
        """Undo stack'e mevcut görseli ekle. Maksimum 20 adım saklanır."""
    ...
    def on_settings_changed(self) -> None:
        ...
        300ms debounce ile gereksiz yeniden render önlenir."""
    ...
    def export_as_zip(self) -> None:
        ...
        # Buffer snapshot alınarak thread güvenliği sağlanır, __rembg__ anahtarları hariç tutulur
        buffer_snapshot = {
    ...
    def export_as_pdf(self) -> None:
        ...
        # Thread güvenliği için buffer değerleri önceden kopyalanır
